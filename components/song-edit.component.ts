...
@Component({
  selector: 'song-edit', //esta es el tag <artist-list>
...
export class SongEditComponent implements OnInit{
  ...
  onSubmit(){
    ...
    this._route.params.forEach((params: Params)=>{ //rescatamos de url
      let id = params['id']; //saca de url el id de la cancion a modificar
        ...
        response =>{
          if(!response.song){
            ...
            if(!this.filesToUpload){
              this._router.navigate(['/album',response.song.album]); //redireccionamiento

            }else{
              //subir e fichero de audio
  ...
  fileChangeEvent(fileInput: any){
    this.filesToUpload = <Array<File>>fileInput.target.files;//va a guardar en un array de tipo file
  }
